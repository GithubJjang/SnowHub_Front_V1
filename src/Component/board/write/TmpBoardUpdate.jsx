...
const TmpBoardUpdate = ()=>{
    ...
    // ReactQuill: 여기서 복잡하더라도 추가를 해야 버튼을 원하는 위치에 붙일수 있음. 아니면 할 방법이x( title, content 어떻게 반환??? )
            ...
            useEffect(()=>{
              ...
              // base64 인코딩 방식은 너무 길기 때문에 부적합 -> Firebase Storage를 쓰자.
                //axios.get('http://localhost:8000/api/v1/tmpboard/'+boardId.id,
                axios.get('https://192.168.55.203:443/api/v1/tmpboard/'+boardId.id,
                    {
                        //params: {name: name},
    ...
    const sendCallback = useCallback((title,content,category)=>{
        ...
        //axios.put('http://localhost:8000/api/v1/tmpboard',formData,
        axios.put('https://192.168.55.203:443/api/v1/tmpboard',formData,
          {
            //params: {name: name},
            headers: {Authorization: 'Bearer '+getCookie('IdTokenCookie'),
            //"Access-Control-Allow-Origin": 'http://localhost:3000',
    ...
    const deleteCallback = useCallback((title,content,category)=>{ // 그거 컴포넌트 destroy써봐야겠다.
        ...
        //axios.delete('http://localhost:8000/api/v1/tmpboard/'+boardId.id,
        axios.delete('https://192.168.55.203:443/api/v1/tmpboard/'+boardId.id,
          {
            //params: {name: name},
            headers: {Authorization: 'Bearer '+getCookie('IdTokenCookie'),
            //"Access-Control-Allow-Origin": 'http://localhost:3000',
